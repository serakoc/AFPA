...
INNER JOIN chambre ON hot_id=cha_hot_id WHERE cha_capacite > 1;

SELECT cli_nom,res_date,hot_nom FROM reservation,hotel,client,chambre
...
SELECT cli_nom,hot_nom,res_date_debut, res_date FROM hotel,reservation,client,chambre
WHERE cli_id = res_cli_id AND ( hot_id = cha_hot_id AND cha_id = res_cha_id);

/* --------------- LOT 3 ---*/

SELECT COUNT(hot_sta_id) FROM hotel WHERE hot_sta_id = 1;
...
res_cha_id = cha_id AND cha_hot_id = hot_id;

SELECT avg(DATEDIFF(res_date_fin, res_date_debut)) FROM reservation,hotel,chambre WHERE hot_sta_id = 1 AND res_cha_id = cha_id AND cha_hot_id = hot_id;
